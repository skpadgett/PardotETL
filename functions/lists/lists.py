...
#
    while i <=p.lists.query()['total_results'] -1: 
        data=p.lists.query(format='json',sort_by='id',id_greater_than=maxid)
        maxid=data['list'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("test/lists/" + "lists_" + writetime + ".json", data)
