# Pardot Extract - visitor activities created_after='yesterday',created_before='today'
#
...
def visitoractivities_func(event, context):
...
#Create PardotAPI class instance
    ...
    while i <=p.visitoractivities.query(created_after='yesterday',created_before='today')['total_results'] -1: 
        data=p.visitoractivities.query(format='json',sort_by='id',created_after='yesterday',created_before='today',id_greater_than=maxid)
        maxid=data['visitoryactivity'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("test/visitoractivities/" + "visitoractivities_" + writetime + ".json", data)
