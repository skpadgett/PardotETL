...
#Create PardotAPI class instance
    ...
    while i <=p.tags.query(id_greater_than=maxid)['total_results'] -1: 
        data=p.tags.query(format='json',sort_by='id',id_greater_than=maxid)
        maxid=data['tag'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("tags/" + "tags_" + writetime + ".json", data)
