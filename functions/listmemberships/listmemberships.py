# Pardot Extract - listmemberships created_after='yesterday',created_before='today'
...
def listmemberships_func(event, context):
...
#Create PardotAPI class instance
    ...
    while i <=p.listmemberships.query(created_after='yesterday',created_before='today')['total_results'] -1: 
        data=p.listmemberships.query(format='json',sort_by='id',created_after='yesterday',created_before='today',id_greater_than=maxid)
        maxid=data['list_membership'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("listmemberships/" + "listmemberships_" + writetime + ".json", data)
        sleep(1)   # 1 sec delay to insure different filenames
        i=i+200

if __name__ == '__main__':
    listmemberships_func()        
