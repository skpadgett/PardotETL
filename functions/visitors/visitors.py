# Pardot Extract - Visitors (created_after='yesterday',created_before='today')
...
def visitors_func(event, context):
...
#Create PardotAPI class instance
    ...
    while i <=p.visitors.query(created_after='yesterday',created_before='today')['total_results'] -1: 
        data=p.visitors.query(format='json',sort_by='id',created_after='yesterday',created_before='today',id_greater_than=maxid)
        maxid=data['visitor'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("visitors/" + "visitors_" + writetime + ".json", data)
