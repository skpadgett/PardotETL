# Pardot Extract - opportunities created_after='yesterday',created_before='today'
...
def opportunities_func(event, context):
...
#Create PardotAPI class instance
    ...
    while i <=p.opportunities.query(created_after='yesterday',created_before='today')['total_results'] -1: 
        data=p.opportunities.query(format='json',sort_by='id',created_after='yesterday',created_before='today',id_greater_than=maxid)
        maxid=data['opportunity'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("eopportunities/" + "opportunities_" + writetime + ".json", data)
