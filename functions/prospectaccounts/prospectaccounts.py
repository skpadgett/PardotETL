# Pardot Extract - prospectaccounts created_after='yesterday',created_before='today'
#
...
def prospectaccounts_func(event, context):
...
#Create PardotAPI class instance
    ...
    while i <=p.prospectaccounts.query(created_after='yesterday')['total_results'] -1: 
        data=p.prospectaccounts.query(format='json',sort_by='id',created_after='yesterday',id_greater_than=maxid)
        maxid=data['prospectAccount'][-1]['id']
        writetime=time.strftime("%Y%m%d-%H%M%S") 
        jsbucket.dump("prospectaccounts/" + "prospectaccounts_" + writetime + ".json", data)
        sleep(1)   # 1 sec delay to insure different filenames
        i=i+200	

if __name__ == '__main__':
    prospectaccounts__func()         
