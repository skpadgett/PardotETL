...
#while i <=p.opportunities.query(created_before='this_month')['total_results'] -1: 
#  data=p.opportunities.query(format='json',sort_by='id',created_before='this_month',id_greater_than=maxid)
#  maxid=data['opportunity'][-1]['id']
#  writetime=time.strftime("%Y%m%d-%H%M%S") 
#  jsbucket.dump("test/opportunities/" + "opportunities_" + writetime + ".json", data)
#  sleep(1)   # 1 sec delay to insure different filenames
#  i=i+200	
#"arguments": {"prospect_updated_after": "2019-07-01 00:00:01","prospect_updated_before": "2020-04-15 00:00:01"}}

#First execution on 4/15/2021
#while i <=p.opportunities.query(created_before='2020-04-15 00:00:01')['total_results'] -1: 
#  data=p.opportunities.query(format='json',sort_by='id',created_before='2020-04-15 00:00:01',id_greater_than=maxid)
#  maxid=data['opportunity'][-1]['id']
#  writetime=time.strftime("%Y%m%d-%H%M%S") 
#  jsbucket.dump("opportunities/" + "opportunities_" + writetime + ".json", data)
#  sleep(1)   # 1 sec delay to insure different filenames
#  i=i+200
#Second execution on 4/15/2021
while i <=p.opportunities.query(created_after='2020-04-15 00:00:01',created_before='2021-04-15 00:00:01')['total_results'] -1: 
  data=p.opportunities.query(format='json',sort_by='id',created_after='2020-04-15 00:00:01',created_before='2021-04-15 00:00:01',id_greater_than=maxid)
  maxid=data['opportunity'][-1]['id']
  writetime=time.strftime("%Y%m%d-%H%M%S") 
  jsbucket.dump("opportunities/" + "opportunities_" + writetime + ".json", data)
  sleep(1)   # 1 sec delay to insure different filenames
